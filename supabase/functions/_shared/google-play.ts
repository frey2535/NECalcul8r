...
export const PRODUCT_PLANS: Record<string, { planKey: string; calculatorLimit: number | null }> = {
...
export const ACTIVE_SUBSCRIPTION_STATES = new Set([
...
export async function fetchSubscriptionStatus(packageName: string, purchaseToken: string, accessToken: string) {
...
/** Sync profile + entitlement from a known purchase token (used by verify + RTDN). */
export async function syncPlayPurchaseForUser(
  client: ReturnType<typeof createClient>,
  userId: string,
  productId: string,
  purchaseToken: string,
  options: { expectedBasePlanId?: string; packageName?: string } = {},
) {
  ...
  const packageName = options.packageName
    ...
    || "com.currentflow.necalcul8r";
  const expectedBasePlanId = options.expectedBasePlanId
  ...
  if (verifiedProductId !== productId) throw new Error("Purchase product ID did not match selected product.");
  if (basePlanId !== expectedBasePlanId) throw new Error("Purchase base plan did not match monthly plan.");
  ...
  // If expired but still within paid period (expiry in future), keep active until expiry.
  const stillPaidPeriod = Boolean(expiresAt) && new Date(String(expiresAt)).getTime() > Date.now();
  const accessActive = active || stillPaidPeriod;

  await client
    .from("profiles")
    .update({
      access_type: "google_play",
      access_status: accessActive ? "active" : "expired",
