...
async function main() {
  ...
  try {
    const { runCommercialLoad2020Baseline } = await server.ssrLoadModule("/src/data/nec/commercialLoad2020Baseline.js");
    const results = runCommercialLoad2020Baseline();
    console.log("");
    console.log("  COMMERCIAL LOAD — 2020 BASELINE");
