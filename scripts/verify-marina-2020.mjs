...
async function main() {
  ...
  try {
    const { runMarinaShorePower2020Baseline } = await server.ssrLoadModule("/src/data/nec/marinaShorePower2020Baseline.js");
    const results = runMarinaShorePower2020Baseline();
    console.log("");
    console.log("  MARINA SHORE POWER — 2020 BASELINE");
