...
export const FIELD_META = {
  ...
  DWELLING_BATHROOM_LOAD_ARTICLE:       { value: "See year",         source: "2017/2020: 220.14(J); 2023: 220.41", description: "Edition-specific reference for bathroom branch-circuit load being included in dwelling general lighting, not added as an extra 1500 VA.", usedBy: ["Dwelling Standard"] },
  ...
  GFCI_EQUIPMENT_SERVICING_RECEPTACLE:  { value: "See year",         source: "210.8(E)",           description: "GFCI for equipment-servicing receptacles installed per 210.63 (HVAC/refrigeration). 2017: null — 210.8(E) did not exist in the 2017 NEC, so the note does not render. 2020: requires GFCI for 210.63 receptacles — pending verification against authorized NFPA 70-2020. 2023/2026: copied from 2020, NOT independently verified. Display/reference only — not used in any numeric formula.", usedBy: ["HVAC Load"] },
  ...
  ISLAND_PENINSULA_ARTICLE:             { value: "See year",         source: "210.52(C)",          description: "Edition-specific island/peninsula reference: 2017 uses 210.52(C)(2)/(C)(3); 2020+ uses 210.52(C)(2).", usedBy: ["Dwelling Standard", "Dwelling Optional", "Receptacle Load"] },
  ...
  ARC_ENERGY_REDUCTION_ARTICLE:         { value: "See year",         source: "240.67 / 240.87",    description: "Year-specific arc-energy-reduction article label. 2017: 240.87 only; 240.67 is not applied because its effective date was January 1, 2020. 2020+: 240.67 / 240.87 where provided by year data.", usedBy: ["Overcurrent Protection"] },

  // ── Conductors & Ampacity ─────────────────────────────────────────
  COPPER_AMPACITY:                      { value: "Table values",     source: "2017: Table 310.15(B)(16); 2020+: Table 310.16", description: "Allowable ampacity for copper conductors in conduit, 60/75/90°C.", usedBy: ["Conductor Ampacity", "Marina Shore Power", "RV Park Load"] },
  ALUMINUM_AMPACITY:                    { value: "Table values",     source: "2017: Table 310.15(B)(16); 2020+: Table 310.16", description: "Allowable ampacity for aluminum conductors in conduit.", usedBy: ["Conductor Ampacity", "Marina Shore Power", "RV Park Load"] },
  ...
  RESISTIVITY:                          { value: "Cu=12.9, Al=21.2",  source: "Engineering constant; NEC Ch.9 Table 8 provides CM", description: "K values are engineering resistivity constants used with NEC conductor circular-mil areas; they are not specifically listed in the NEC.", usedBy: ["Voltage Drop", "Marina Shore Power", "RV Park Load"] },
  ...
  RECEPTACLE_DEMAND_TIERS:              { value: "100%/50%",         source: "2017/2020: 220.44; 2023: 220.47", description: "Non-dwelling receptacle demand factors: first 10 kVA at 100%, remainder at 50%.", usedBy: ["Commercial Load", "Receptacle Load", "Demand Factor"] },
  RECEPTACLE_DEMAND_ARTICLE:            { value: "See year",         source: "Article 220 reorganization", description: "Edition-specific receptacle demand reference: 2017/2020 use 220.44; 2023 uses 220.47.", usedBy: ["Commercial Load", "Receptacle Load", "Demand Factor"] },
  RECEPTACLE_DEMAND_TABLE:              { value: "See year",         source: "Article 220 reorganization", description: "Edition-specific table/reference label for non-dwelling receptacle demand.", usedBy: ["Commercial Load", "Receptacle Load", "Demand Factor"] },
  HEAVY_DUTY_LAMPHOLDER_ARTICLE:        { value: "220.14(E)",        source: "NEC 220.14(E)", description: "Heavy-duty lampholders heading; tracked separately so sign/show-window loads are not assigned to 220.14(E).", usedBy: ["Commercial Load"] },
  SIGN_OUTLET_ARTICLE:                  { value: "220.14(F)",        source: "NEC 220.14(F)", description: "Sign and outline lighting outlet minimum-load reference.", usedBy: ["Commercial Load"] },
  SHOW_WINDOW_ARTICLE:                  { value: "220.14(G)",        source: "NEC 220.14(G)", description: "Show-window load reference: 200 VA per linear foot.", usedBy: ["Commercial Load"] },
  OFFICE_RECEPTACLE_MIN_ARTICLE:        { value: "220.14(K)",        source: "NEC 220.14(K)", description: "Office receptacle minimum reference; 2017 heading includes banks, 2020/2023 heading is Office Buildings.", usedBy: ["Commercial Load"] },
  OFFICE_RECEPTACLE_MIN_HEADING:        { value: "See year",         source: "NEC 220.14(K)", description: "Edition-specific 220.14(K) heading text.", usedBy: ["Commercial Load"] },
  OFFICE_RECEPTACLE_MIN_VA_PER_SQFT:    { value: "1 VA/ft²",         source: "NEC 220.14(K)", description: "Office receptacle minimum load compared against receptacle demand.", usedBy: ["Commercial Load"] },
  SHOW_WINDOW_VA_PER_FOOT:              { value: "200 VA/ft",        source: "NEC 220.14(G)", description: "Show-window load per linear foot.", usedBy: ["Commercial Load"] },
  SIGN_OUTLET_MIN_VA:                   { value: "1200 VA",          source: "NEC 220.14(F)", description: "Minimum sign and outline lighting load when present.", usedBy: ["Commercial Load"] },
