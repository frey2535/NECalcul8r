          ...
          {headers && headers.length > 0 && (
            <thead>
              <tr className="bg-slate-50 border-b-2 border-slate-200">
                {headers.map((h, i) => (
                  <th key={i} className={cn("px-3 font-bold text-slate-600 uppercase tracking-wide whitespace-nowrap sticky top-0 bg-slate-50 z-10 shadow-[0_1px_0_0_rgba(0,0,0,0.05)]", compact ? "py-1.5 text-[9px]" : "py-2 text-[10px]")}>
                    {h}
                  </th>
                ))}
              </tr>
          ...
          <tbody>
            {rows.map((row, ri) => (
              <tr key={ri} className={cn("border-b border-slate-100 last:border-0 transition-colors duration-150 hover:bg-blue-50/70", ri % 2 === 0 ? "bg-white" : "bg-slate-50/50")}>
                {row.map((cell, ci) => (
                  <td key={ci} className={cn("px-3 text-slate-700 whitespace-nowrap font-medium", compact ? "py-1.5 text-[10px]" : "py-2 text-xs")}>
                    {cell}
                  </td>
                ))}
              </tr>
