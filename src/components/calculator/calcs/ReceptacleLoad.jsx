...
export default function ReceptacleLoad({ category, necYear = "2023" }) {
  ...
  const { totalConnected_VA: totalVA, demandAdjusted_VA: demandVA,
    totalAmps, demandAmps, circuits_required: circuitsNeeded, receptacleDemandArticle, steps } = r;
  ...
  return (
    <CalcLayout category={category} necYear={necYear} inputValues={v} outputValues={r} result={
      <div className="space-y-2">
        ...
        <ResultSection title={`After Demand Factor (NEC ${receptacleDemandArticle})`}>
        ...
        <NoteBox>
          <ul className="list-disc pl-3.5 space-y-1">
            ...
            <li>NEC {necYear} {receptacleDemandArticle}: Commercial demand — {nec.RECEPTACLE_DEMAND_TIERS.map((t) => `${(t.factor * 100).toFixed(0)}%${t.band < Infinity ? ` first ${t.band.toLocaleString()} VA` : " remainder"}`).join(", ")}.</li>
      ...
      <Field label={`Apply Demand Factor (NEC ${receptacleDemandArticle})?`}>
        <Select value={String(v.applyDemand)} onChange={val => setV(p => ({ ...p, applyDemand: val }))} options={[
          { value: "true", label: `Yes — Commercial (${receptacleDemandArticle})` },
