...
const FORMULAS = [
  { label: "Conductor Ampacity (NEC 630.11)", formula: "I_conductor = I_rated × √(DC% / 100)", description: "DC = duty cycle percentage; √(DC/100) is the multiplier from Table 630.11" },
    ...
    <CalcLayout category={category} necYear={necYear} inputValues={v} outputValues={r} result={
      <div className="space-y-2">
        <ResultSection title="Welder Load (NEC 630.11)">
          ...
          <ResultRow label="DC Multiplier (√DC/100)" value={dcFactor.toFixed(3)} />
        ...
        <ResultSection title="Conductor & OCPD Sizing">
          <ResultRow label="Min Conductor Ampacity" value={conductorA.toFixed(1)} unit="A" highlight
            sub="NEC 630.11: Nameplate × √(DC%/100)" />
        ...
        <NoteBox>NEC {necYear} 630.11(A): Arc welder conductor sized at nameplate primary current × √(duty cycle / 100). NEC 630.12: OCPD ≤ 200% of the rated primary current of the welder. For multiple welders, NEC 630.11(B) applies.</NoteBox>
      </div>
    }>
      ...
      <Field label="Duty Cycle" unit="%" hint="From nameplate; typical: 20–100%">
        <Select value={v.dutyCycle} onChange={set("dutyCycle")} options={[
          ...
          { value: 50, label: "50%" }, { value: 60, label: "60%" }, { value: 80, label: "80%" },
