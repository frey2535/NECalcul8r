...
export function calcWelderLoad(v, nec) {
  ...
  const dcFactor = exact
    ? exact.mult
    : (table.filter((r) => r.dc <= DC).sort((a, b) => b.dc - a.dc)[0]?.mult
  ...
  const steps = [
    { label: "Duty Cycle Multiplier (Table 630.11)", formula: "Mult = √(duty cycle ÷ 100)", expression: `√(${DC} ÷ 100)`, result: Math.round(dcFactor * 1000) / 1000, note: `${DC}% duty cycle` },
