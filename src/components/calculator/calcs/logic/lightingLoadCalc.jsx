/**
 * Pure calculation logic for Lighting Load (NEC Article 220; references vary by code cycle).
 */
...
function lightingLoadCite(nec, occupancyKey) {
  ...
  if (nec.OCCUPANCY_UNIT_LOAD_ARTICLES?.[occupancyKey]) {
    return nec.OCCUPANCY_UNIT_LOAD_ARTICLES[occupancyKey];
  }
...
export function calcLightingLoad(v, nec) {
  ...
  const demandNote = demandCfg?.tiers
    ? `${lightingDemandTable} ${demandKey} tiers applied`
    : `${Math.round(nec_VA)} @ 100% (occupancy not listed in ${lightingDemandTable})`;
  ...
  const steps = [
    ...
    { label: `Demand-Adjusted Load (${lightingDemandTable})`, formula: "Demand = tiered demand factors by occupancy", expression: demandNote, result: Math.round(demand), unit: "VA" },
