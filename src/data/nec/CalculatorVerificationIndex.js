...
const SOURCE_FILES = {
  ...
  multifamily_standard: "components/calculator/calcs/MultifamilyStandard.jsx",
...
const LOGIC_FILES = {
  ...
  multifamily_standard: "logic/multifamilyStandardCalc.jsx",
...
export const VERIFICATION_RESULTS = {
  ...
  pull_box_sizing: {
    ...
    limitations: [
      "Previously identified defects have been corrected and verified per user review: (1) applicability based on conductor size 4 AWG+, (2) 314.28(C) universal 4×4×2 minimum removed, (3) spacing uses 6× largest raceway, (4) equal-sized raceways included in 'additional' sum, (5) single raceway requires path mapping, (6) conductor-path mapping added, (7) row identification added, (8) splice sizing belongs to 314.28(A)(2), (9) spacing only between connected entries, (10) 314.28(B) clarified as conductor cabling/racking for boxes over 6 ft.",
      "Regression tests use independent expected values hand-calculated from NEC 314.28 text (TEST-A through TEST-G plus PB-09B splice control). All 21 tests pass. Previous tests were insufficient — expected values derived from production logic — and have been replaced.",
    ...
    completeDependencyList: [
      ...
      "314.28(A)(2) (angle/U pulls or splices — 6× largest in row + sum of additional in same row; same-row spacing = 6× largest in row)",
      "314.28(B) (conductors in pull or junction boxes — cable/rack conductors if any dimension exceeds 6 ft)",
    ],
    ruleVerification: {
      ...
      "314.28(A)(1)": { status: "VERIFIED", detail: "Straight pull: 8× largest raceway. Logic verified — 21/21 regression tests pass." },
      "314.28(A)(2)": { status: "VERIFIED", detail: "Angle/U/splice sizing: 6× largest in row + sum of ALL additional raceways in same row (including equal-sized). Connected-entry spacing = 6× largest in path. Straight-pull boxes with splices are checked under both 314.28(A)(1) and (A)(2)." },
      "314.28(B)": { status: "VERIFIED", detail: "Conductors in pull or junction boxes: if any dimension exceeds 6 ft, conductors must be cabled or racked in an approved manner." },
    ...
    consumers: [
      ...
      { file: "src/data/nec/pullBoxSizingRegression.js", type: "regression tests", status: "verified — 21/21 pass with independent expected values" },
    ...
    defectsFound: [
      ...
      {
        severity: "minor",
        description: "Splice handling was attributed to 314.28(B), but splice sizing belongs under 314.28(A)(2).",
        fix: "Added an explicit splice toggle. Where splices are present, the calculator applies 314.28(A)(2) row dimensions in addition to straight-pull sizing and clarifies 314.28(B) as the conductor cabling/racking rule for boxes over 6 ft.",
        filesFixed: ["src/components/calculator/calcs/logic/pullBoxSizingCalc.js", "src/components/calculator/calcs/PullBoxSizing.jsx"],
        status: "FIXED",
      },
      {
    ...
    confidenceNotes: {
      runtimeFormula: "Logic corrected and verified per user review. All 21 regression tests pass with independent expected values.",
      regression: "Regression tests use independent expected values hand-calculated from NEC 314.28 text (TEST-A through TEST-G plus PB-09B splice control). All 21 tests pass. Previous tests were insufficient — expected values derived from production logic — and have been replaced.",
  ...
  commercial_load: {
    ...
    completeDependencyList: [
      ...
      "2017/2020 220.44; 2023 220.47 (receptacle demand — 100% first 10k + 50% remainder)",
      "220.14(E) (heavy-duty lampholders)",
      "220.14(F) (sign and outline lighting — min 1200 VA)",
      "220.14(G) (show windows — 200 VA/linear foot)",
      "220.14(K) (2017 heading: Banks and Office Buildings; 2020/2023 heading: Office Buildings — 1 VA/sq ft minimum)",
    ...
    ruleVerification: {
      ...
      "220.44 / 220.47": { status: "VERIFIED", detail: "100% first 10,000 VA + 50% remainder correctly implemented via RECEPTACLE_DEMAND_TIERS. 220.44 applies to 2017/2020; 2023 moved this rule to 220.47." },
      "220.14(E)": { status: "VERIFIED", detail: "Heavy-duty lampholders heading tracked so it is not used for sign/show-window loads." },
      "220.14(F)": { status: "VERIFIED", detail: "Sign and outline lighting load uses max(user-entered VA, 1200 VA) when a sign load is entered." },
      "220.14(G)": { status: "VERIFIED", detail: "200 VA/linear foot for show windows correctly implemented." },
      "220.14(K)": { status: "VERIFIED", detail: "Office 1 VA/sq ft receptacle minimum is compared against the receptacle demand result; the larger value is used. 2017 heading reads Banks and Office Buildings; 2020/2023 heading reads Office Buildings." },
    ...
    confidenceNotes: {
      ...
      table: "Table 220.42 demand factors verified for all 4 listed occupancies. Fallback to 100% for non-listed occupancies correct per NEC. Receptacle demand verified as 220.44 for 2017/2020 and 220.47 for 2023.",
    },
    runtimeDependenciesVerified: [
      ...
      "RECEPTACLE_DEMAND_TIERS (2017/2020 220.44; 2023 220.47 — 100%/50% tiers)",
      "Heavy-duty lampholders label (220.14(E))",
      "Sign and outline lighting minimum 1200 VA when entered (220.14(F))",
      "Show windows 200 VA/ft (220.14(G))",
      "Receptacle 180 VA/yoke (220.14(I))",
      "Office receptacle minimum 1 VA/sq ft (220.14(K); 2017 heading also names banks)",
    ],
    ...
    annexDCoverage: {
      ...
      details: "No Annex D example covers the Commercial Load calculator (220.12/220.42/220.44 or 2023 220.47). Annex D examples D1-D5 cover dwelling and multifamily calculations. Commercial load verification relies on direct NEC rule verification, hand calculations, boundary tests, and four-year regression testing.",
    },
  },
};
